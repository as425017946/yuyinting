...
import 'hz_room_more_liushui_page.dart';

///厅流水 - 新增
class HZRoomLiuShuiPage extends StatefulWidget {
  String ghID;
  HZRoomLiuShuiPage({super.key, required this.ghID});

  @override
  State<HZRoomLiuShuiPage> createState() => _HZRoomLiuShuiPageState();
}

class _HZRoomLiuShuiPageState extends State<HZRoomLiuShuiPage> {
  var appBar;
  ...
  void initState() {
    ...
    super.initState();
    appBar = WidgetUtils.getAppBar('厅流水', true, context, false, 0);
    ...
    return GestureDetector(
      onTap: (() {
        if(MyUtils.checkClick()){
          MyUtils.goTransparentPageCom(context, HZRoomMoreLiuShuiPage(ghID: listInfo[i].id.toString(),));
        }
      }),
      child: Container(
        ...
        child: Column(
          children: [
            ...
            WidgetUtils.commonSizedBox(10, 10),
            WidgetUtils.onlyTextCenter('查房间流水>', StyleUtils.getCommonTextStyle(color: MyColors.g3, fontSize: ScreenUtil().setSp(25))),
            WidgetUtils.commonSizedBox(10, 10),
          ],
        ),
      ),
                  ...
                  Row(
                    children: [
                      ...
                      Container(
                        ...
                        child:
                        WidgetUtils.commonTextField(controller, '输入厅ID或名称'),
                      ),
                    ],
                  ),
  ...
  Future<void> doPostRoomSpendingList() async {
    ...
    try {
      hzTingBean bean = await DataUtils.postGuildSpendingList(params);
      switch (bean.code) {
