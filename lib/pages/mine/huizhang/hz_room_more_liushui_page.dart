...
///房间流水 - 新增
class HZRoomMoreLiuShuiPage extends StatefulWidget {
  String ghID;
  HZRoomMoreLiuShuiPage({super.key, required this.ghID});

  @override
  State<HZRoomMoreLiuShuiPage> createState() => _HZRoomMoreLiuShuiPageState();
}

class _HZRoomMoreLiuShuiPageState extends State<HZRoomMoreLiuShuiPage> {
  var appBar;
  ...
  void initState() {
    ...
    super.initState();
    appBar = WidgetUtils.getAppBar('厅房间流水', true, context, false, 0);
  ...
  Future<void> doPostRoomSpendingList() async {
    ...
    try {
      roomLiuShuiBean bean = await DataUtils.postCroomSpendingList(params);
      switch (bean.code) {
