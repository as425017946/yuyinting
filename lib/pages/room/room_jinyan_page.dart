...
/// 厅内禁言列表
class RoomJinYanPage extends StatefulWidget {
  String roomID;
  RoomJinYanPage({super.key, required this.roomID});

  @override
  State<RoomJinYanPage> createState() => _RoomJinYanPageState();
}

class _RoomJinYanPageState extends State<RoomJinYanPage> {
  @override
  void initState() {
    // TODO: implement initState
    super.initState();
    doPostRoomForbationList();
  }
  /// 在线用户推荐使用
  ...
  List<Data> list = [];
  /// 房间管理员列表
  Future<void> doPostRoomForbationList() async {
    Map<String, dynamic> params = <String, dynamic>{
      'room_id': widget.roomID,
    };
    try {
      managerBean bean = await DataUtils.postRoomForbationList(params);
      switch (bean.code) {
  ...
  /// 设置/取消房间用户禁言
  Future<void> doPostSetRoomForbation(String uid,int index) async {
    Map<String, dynamic> params = <String, dynamic>{
      'uid': uid,
      'status': '0',
      'room_id': widget.roomID,
    ...
    try {
      ...
      switch (bean.code) {
        case MyHttpConfig.successCode:
          MyToastUtils.showToastBottom("已成功移除禁言");
          setState(() {
            list.removeAt(index);
          });
          break;
